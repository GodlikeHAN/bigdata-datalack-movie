...
from streaming.realtime_indexer import index_realtime_events, normalize_realtime_events


def consume_trending_events(max_messages: int | None = None, timeout_ms: int = 15000) -> dict:
    consumer = KafkaConsumer(
        KAFKA_TRENDING_TOPIC,
        bootstrap_servers=KAFKA_BOOTSTRAP_SERVERS,
        group_id=KAFKA_TRENDING_CONSUMER_GROUP,
    ...
    normalized_events = normalize_realtime_events(events)
    ...
    write_json_lines(output_file, normalized_events)

    indexed = index_realtime_events(normalized_events)
    return {
        "stored_events": len(normalized_events),
